...
openHdb fn = do
    fd <- withCString fn $ \cfn -> c_open cfn o_rdwr
    when (fd < 0) (fail "openHdb: open failed")
    cstr <- mallocBytes 4096 :: IO CString
    rc <- c_read fd cstr (4096)
    if (rc>0) then do
	cbuf <- peekCString cstr
	let cblk = read cbuf :: Cblk
        return $ hdb0{zfd=fd, zfn=fn, zblk=cblk, zbuf=cstr}
    else do
	free cstr
        fail "openHdb: read Cblk failed"

--closeHdb StateT Hdb IO ()
closeHdb hdb = do
    c_close $ zfd hdb
